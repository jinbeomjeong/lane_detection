...
class SCNN(nn.Module):
    ...
    def forward(self, img, seg_gt=None, exist_gt=None):
        x = self.backbone(img)
        x = self.post_processing_layer(x)
        x = self.layer1(x)
    ...
    def net_init(self, input_size, ms_ks):
        ...
        for params in self.backbone.parameters():
            param_list.append(params.size())

        last_num_channels = param_list[-1][0]

        self.post_processing_layer = nn.Sequential(nn.ConvTranspose2d(last_num_channels, 512, 2, 2),
                                                   nn.BatchNorm2d(512),
                                                   nn.ReLU(),
        ...
        # ----------------- SCNN part -----------------
        self.layer1 = nn.Sequential(nn.Conv2d(256, 1024, 3, padding=4, dilation=4, bias=False),
                                    nn.BatchNorm2d(1024),
                                    nn.ReLU(),
        ...
        self.message_passing.add_module('up_down', nn.Conv2d(128, 128, (1, ms_ks),
                                                             padding=(0, ms_ks // 2), bias=False))
        self.message_passing.add_module('down_up', nn.Conv2d(128, 128, (1, ms_ks),
                                                             padding=(0, ms_ks // 2), bias=False))
        self.message_passing.add_module('left_right', nn.Conv2d(128, 128, (ms_ks, 1),
                                                                padding=(ms_ks // 2, 0), bias=False))
        self.message_passing.add_module('right_left', nn.Conv2d(128, 128, (ms_ks, 1),
                                                                padding=(ms_ks // 2, 0), bias=False))
        # (nB, 128, 36, 100)

        # ----------------- SCNN part -----------------
        self.layer2 = nn.Sequential(nn.Dropout2d(0.1),
                                    nn.Conv2d(128, 5, 1))  # get (nB, 5, 36, 100)

        self.layer3 = nn.Sequential(nn.Softmax(dim=1),  # (nB, 5, 36, 100)
